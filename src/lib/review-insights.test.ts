...
describe("review insights", () => {
  it("builds material coverage and gap guidance", () => {
    ...
    expect(insights.totalMaterials).toBe(materials.length);
    expect(insights.topMaterial?.boundJobCount).toBe(Math.max(...insights.materials.map((material) => material.boundJobCount)));
